...
setlocal
set "ROOT=%~dp0"
set "INDEX=%ROOT%de.html"

rem Convert the local path to a file:// URL and open the admin view.
for /f "usebackq delims=" %%I in (`powershell -NoLogo -NoProfile -Command "[uri]::new((Resolve-Path '%INDEX%')).AbsoluteUri + '?admin=1'"`) do set "URL=%%I"
